...
grep -Fq "SCRIPT_VERSION='2026.09.05.3'" "$script"
...
grep -Fq 'bubblewrap' "$work/ubuntu.sh"
grep -Fq "command -v bwrap" "$work/ubuntu.sh"
! grep -Fq -- '--bind "$HOME/storage/shared:/sdcard"' "$work/cx-wrapper.sh"
grep -Fq 'cd /sdcard; exec codex' "$work/cx-wrapper.sh"
grep -Fq '支持 https:// 或 http://' "$work/cx-inner.sh"
grep -Fq '确认仍要使用 HTTP？请输入 y' "$work/cx-inner.sh"
grep -Fq 'API Key 和对话内容可能被同一网络中的其他设备截获' "$work/cx-inner.sh"
grep -Fq 'API Key（可留空）' "$work/cx-inner.sh"
grep -Fq '已按无 API Key 模式配置' "$work/cx-inner.sh"
grep -Fq '未填写 API Key，已跳过自动获取模型列表' "$work/cx-inner.sh"
grep -Fq "if [ \"\$kind\" = 12 ]" "$work/cx-inner.sh"
grep -Fq '请手动输入模型名称' "$work/cx-inner.sh"
grep -Fq "else: t.pop('env_key',None)" "$work/cx-inner.sh"
grep -Fq '"${headers[@]}"' "$work/cx-inner.sh"
grep -Fq "curl --proto '=http,https'" "$work/cx-inner.sh"
! grep -Fq '\n  say' "$script"
