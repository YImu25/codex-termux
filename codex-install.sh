...
for line in open(sys.argv[1]):
    parts=line.rstrip('\n').split('\t',1)
    if len(parts)==2 and parts[0] and parts[1] and parts not in rows: rows.append(parts)
try: print('\t'.join(rows[int(sys.argv[2])-1]))
except (ValueError,IndexError): raise SystemExit(2)
PY
}
hide_remote_model() {
 local n row pid name url key token model current
...
PY
 ...
 rm -f "$tmp" "$parsed"
 [ "${#REMOTE_MODELS[@]}" -gt 0 ] || {
   echo '接口没有可显示的模型：可能未返回标准列表，或模型均已隐藏。'
   return 1
 }
 return 0
}
...
restore_hidden_model() {
 local n row p m
 echo; echo '────────── 恢复隐藏模型 ──────────'
 if [ ! -s "$HIDDEN_MODELF" ]; then echo '  （暂无隐藏模型）'; return 0; fi
 show_hidden_models
 echo '  （0）返回'
 read -rp '输入要恢复的模型序号：' n; [ "$n" = 0 ] && return 0
